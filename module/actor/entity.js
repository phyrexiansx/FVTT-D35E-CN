...
import {targetAutoSettleCheck, getRollAdvantageMode} from '../automation/intuitive.js';
...
export class ActorPF extends Actor {
  ...
  prepareData() {
    ...
    // 最终触及（尺）= 基础（traits.reach）+ 变化加成（traits.reachBonus），最低 5 尺（相邻格）
    const _reachBase = parseInt(preparedData.traits?.reach) || 0;
    const _reachBonus = parseInt(preparedData.traits?.reachBonus) || 0;
    preparedData.traits.reachTotal = 5 + _reachBase + _reachBonus;
    ...
    actorData.items.filter((obj) => {
      ...
      if (cls.system.classType !== 'racial' && cls.system.classType !== 'minion')
  ...
  async createAttackFromWeapon(item) {
    ...
    if (identified) {
      _enhancements.forEach((i) => {
        ...
        const wd = enhancementData.weaponData || {};
        // 防御：基础武器无伤害公式时 damage.parts 从未初始化，静态伤害 push 会抛错并中断后续所有附魔效果
        if (!Array.isArray(attackData['system.damage.parts'])) attackData['system.damage.parts'] = [];
        let conditional = Item35E.defaultConditional;
        conditional.name = i.name;
        conditional.default = false;
        if (wd.damageRoll !== '') {
          if (wd.optionalDamage) {
            let damageModifier = Item35E.defaultConditionalModifier;
            damageModifier.formula = wd.damageRoll;
            damageModifier.type = wd.damageTypeId;
            damageModifier.target = 'damage';
            damageModifier.subTarget = 'allDamage';
            conditional.modifiers.push(damageModifier);
          } else {
            if (wd.damageRoll !== undefined &&
                wd.damageRoll !== null)
        ...
        if (wd.attackRoll !== '') {
          if (wd.optionalDamage) {
            ...
            attackModifier.formula = wd.attackRoll;
            attackModifier.target = 'attack';
            attackModifier.subTarget = 'allAttack';
            conditional.modifiers.push(attackModifier);
          } else {
            if (wd.attackRoll !== undefined &&
                wd.attackRoll !== null) {
              // 防御：基础武器无攻击公式时 attackBonus 从未赋值（undefined），直接拼接会产生 "undefined + 2" 导致掷骰报错
              attackData['system.attackBonus'] = attackData['system.attackBonus']
                ? attackData['system.attackBonus'] + ' + ' + wd.attackRoll
                : wd.attackRoll;
            }
          }
        }
  ...
  async rollSpellResistance(spellPenetration, options = {}) {
    if (!this.testUserPermission(game.user, 'OWNER'))
      return ui.notifications.warn(
          game.i18n.localize('D35E.ErrorNoActorPermission'));
    return this.rollSpellPowerResistance(spellPenetration, 'sr', options);
  }

  async rollSpellPowerResistance(spellPenetration, type, options = {}) {
    const _roll = async function(type, form, props, autoRollMode) {
      let spellPenetrationTotal = spellPenetration,
          optionalFeatIds = [],
          optionalFeatRanges = new Map(),
          rollMode = autoRollMode || null;
      let resistanceManualBonus = 0;
      let advMode = 'normal'; // [D35E-CN-ADV]优势/劣势
      // Get data from roll form
      if (form) {
        resistanceManualBonus = form.find('[name="res-bonus"]').val() || 0;

        rollMode = form.find('[name="rollMode"]').val();
        // [D35E-CN-ADV]对话框「检定方式」三选一
        const advModeEl = form.find('[name="adv-mode"]:checked');
        if (advModeEl.length) advMode = advModeEl.val();
      ...
      // [D35E]优势/劣势：对话框radio（显式）> Alt/Ctrl修饰键 >变化效果标签（抵消）>普通
      if (advMode === 'normal') {
        if (options?.event?.altKey) advMode = 'advantage';
        else if (options?.event?.ctrlKey) advMode = 'disadvantage';
        else {
          const _tagMode = getRollAdvantageMode(this);
          if (_tagMode === 1) advMode = 'advantage';
          else if (_tagMode === -1) advMode = 'disadvantage';
        }
      }
      // [D35E-CN-ADV]聊天卡上显示优势/劣势
      if (advMode === 'advantage') rollModifiers.push('优势');
      else if (advMode === 'disadvantage') rollModifiers.push('劣势');
      ...
      let roll = new Roll35e(
          (advMode === 'advantage' ? '2d20kh' : (advMode === 'disadvantage' ? '2d20kl' : '1d20')) +
          ' + @spellPenetrationTotal', rollData).roll();
    ...
    //自动法术穿透（直觉自动结算）：跳过对话框，直接以基础穿透检定+被动combat changes掷骰
    if (options.skipDialog) {
      return _roll.call(this, type, null, props, options.rollMode || 'publicroll');
    }
    ...
    if (this.system.attributes[`${type}`].total) {
      let wasRolled = false;
      // [D35E-CN-ADV]优势/劣势：见 _roll 内实现（对话框radio > Alt/Ctrl > 变化效果标签）
     buttons.normal = {
  ...
  async rollSavingThrow(_savingThrow, ability, target, options = {}) {
    ...
    const _roll = async function(
      ...
      // [D35E]优势/劣势：对话框radio（显式）> Alt/Ctrl修饰键 >变化效果标签（抵消）>普通
      let _advMode = options?.event?.altKey ? 1 : (options?.event?.ctrlKey ? -1 : getRollAdvantageMode(this));
      if (form) {
        const advModeEl = form.find('[name="adv-mode"]:checked');
        if (advModeEl.length && advModeEl.val() !== 'normal') _advMode = advModeEl.val() === 'advantage' ? 1 : -1;
      }
      // [D35E-CN-ADV]聊天卡上显示优势/劣势
      if (_advMode === 1) rollModifiers.push('优势');
      else if (_advMode === -1) rollModifiers.push('劣势');
      allCombatChanges = ItemCombatChangesHelper.getAllSelectedCombatChangesForRoll(
          this.items,
          attackType,
          rollData,
      ...
      this._addCombatChangesToRollData(allCombatChanges, rollData);
      let saveRollFormula = _advMode === 1 ? '2d20kh' : (_advMode === -1 ? '2d20kl' : '1d20');
      ...
      modifiersList.unshift(
          {value: roll.terms[0].total, name: '技能检定'});
      ...
      let chatData = {
        speaker: options.speaker ? options.speaker : ChatMessage.getSpeaker(
            {actor: this.data}),
        rollMode: rollMode || 'gmroll',
        sound: options.skipDialog ? null : CONFIG.sounds.dice,
        'flags.D35E.noRollRender': true,
      };
      let chatTemplateData = {
        name: this.name,
    ...
    //自动豁免（直觉自动结算）：跳过对话框，直接以基础豁免+被动combat changes掷骰
    if (options.skipDialog) {
      return _roll.call(this, savingThrowId, savingThrowAbility,
          savingThrowBaseAbility, target, null, props,
          options.rollMode || 'publicroll');
    }
    const html = await renderTemplate(template, dialogData);
    let roll;
    const buttons = {};
  ...
  async rollSkill(skillId, options = {}) {
    ...
    const _roll = async function(
      ...
      // Get data from roll form
      if (form) {
        skillManualBonus = Number(form.find('[name="sk-bonus"]').val()) || 0;
      ...
      // [D35E]优势/劣势：对话框radio（显式）> 按钮强制（2d20kh/kl）> Alt/Ctrl修饰键 >变化效果标签（抵消）>普通；Take10/20无d20自动忽略
      if (skillRollFormula === '1d20') {
        let _advMode = options?.event?.altKey ? 1 : (options?.event?.ctrlKey ? -1 : getRollAdvantageMode(this));
        if (form) {
          const advModeEl = form.find('[name="adv-mode"]:checked');
          if (advModeEl.length && advModeEl.val() !== 'normal') _advMode = advModeEl.val() === 'advantage' ? 1 : -1;
        }
        skillRollFormula = _advMode === 1 ? '2d20kh' : (_advMode === -1 ? '2d20kl' : '1d20');
      } else if (skillRollFormula === '2d20kh' || skillRollFormula === '2d20kl') {
        // 已有按钮强制，radio 显式选择仍可覆盖
        if (form) {
          const advModeEl = form.find('[name="adv-mode"]:checked');
          if (advModeEl.length && advModeEl.val() !== 'normal') {
            skillRollFormula = advModeEl.val() === 'advantage' ? '2d20kh' : '2d20kl';
          }
        }
      }
      // [D35E-CN-ADV]聊天卡上显示优势/劣势
      if (skillRollFormula === '2d20kh') rollModifiers.push('优势');
      else if (skillRollFormula === '2d20kl') rollModifiers.push('劣势');
      allCombatChanges = ItemCombatChangesHelper.getAllSelectedCombatChangesForRoll(
          this.items,
          attackType,
          rollData,
      ...
      let skillSourceDetails = duplicate(this.sourceDetails[sourceSkillId] || []);
      ...
      const templateData = mergeObject(
          chatTemplateData,
          {
            ...
            take10: take10,
            tooltip: renderedTooltip,
            success: target && roll.total >= target && targetAutoSettleCheck(this),
            failed: target && roll.total < target && targetAutoSettleCheck(this),
            target: target,
            properties: props,
            hasProperties: props.length > 0,
          },
    ...
    buttons.normal = {
      ...
      callback: (html) => {
        wasRolled = true;
        roll = _roll.call(this, options.target, html, props, sklName, _skillFormula,
            sourceChangesSkillId, options.rollMode);
      },
    };
    buttons.advantage = {
      label: '优势',
      callback: (html) => {
        roll = _roll.call(this, options.target, html, props, sklName, '2d20kh',
            sourceChangesSkillId, options.rollMode);
      },
    };
    buttons.disadvantage = {
      label: '劣势',
      callback: (html) => {
        roll = _roll.call(this, options.target, html, props, sklName, '2d20kl',
            sourceChangesSkillId, options.rollMode);
      },
    };
    return new Promise((resolve) => {
  ...
  async rollGrapple(target, options = {}) {
    ...
    const _roll = async function(target, form, props) {
      ...
      let roll = new Roll35e(
          (advMode === 'advantage' ? '2d20kh' : (advMode === 'disadvantage' ? '2d20kl' : '1d20')) +
          ' + @grappleModTotal + @grappleManualBonus + @featGrappleBonus',
          rollData).roll();
  ...
  _addCombatChangesToRollData(allCombatChanges, rollData) {
    ...
    allCombatChanges.forEach((change) => {
      if (change.field.indexOf('$') !== -1) {
        ...
        // 修复：公式可能是 @引用或骰子表达式，不能用 parseInt
        const formulaStr = (change.formula != null ? String(change.formula) : '').trim();
        if (formulaStr === '') {
          changeVal = 0;
        } else if (/^-?\d+(\.\d+)?$/.test(formulaStr)) {
          changeVal = Number(formulaStr);
        } else {
          try {
            changeVal = new Roll35e(formulaStr, rollData).roll().total;
            if (!Number.isFinite(changeVal)) changeVal = 0;
          } catch (err) {
            console.error('D35E | 战斗变更公式计算失败:', change.formula, err);
            changeVal = 0;
          }
        }
  ...
  async rollAbilityTest(abilityId, options = {}) {
    ...
    return DicePF.d20Roll({
      ...
      speaker: ChatMessage.getSpeaker({actor: this}),
      template: 'systems/D35E/templates/apps/ability-roll-dialog.html',
      fastForward: false,
      takeTwenty: false,
      chatTemplate: 'systems/D35E/templates/chat/roll-ext.html',
  ...
  async rollDefenseDialog({
    ...
    if (skipDialog) {
      ...
      if (baseConceal > 0) {
        result.conceal = true;
        result.concealOverride = Math.max(result.concealOverride || 0, baseConceal);
      }
      //自动结算的命中/伤害记录对所有人可见（避免默认 gmroll 私聊仅 GM 可见）
      result.rollMode = "publicroll";
      return result;
    }
    // Render modal dialog
  ...
  async createEmbeddedEntity(embeddedName, createData, options = {}) {
    ...
    let linkedItems = [];
    // 记录本次批量创建中已占用的槽位数（避免同槽位装备同时默认穿戴导致超载）
    const _pendingSlotCounts = {};
      ...
      if (obj?.system?.linkedItems && obj.system.linkedItems.length > 0) {
        ...
        for (let data of obj.system.linkedItems) {
          ...
          if (packItem != null) {
            ...
            // D35E-CN: linkedItems 元数据存的是 packId（getLinkDataFromData 写入），data.pack 恒为 undefined
            delete itemData._id; // 不使用 compendium 的 _id，避免重复导入时 id 冲突
            itemData.system.originPack = data.packId ?? data.pack;
    ...
    for (let obj of createData) {
      ...
      // 默认穿戴：新获得的武器/装备默认装备；装备槽已满（含本次批量创建）则默认不装；容器内物品不装备
      if (['weapon', 'equipment'].includes(obj.type)) {
        const inContainer = obj.system.containerId && obj.system.containerId !== "none";
        if (inContainer) {
          obj.system.equipped = false;
        } else if (obj.system.equipped !== true) {
          const slot = ItemEquipHook.getEffectiveSlot(obj);
          if (slot) {
            const used = this.items.filter((i) => i.type === "equipment" && i.system.equipped && ItemEquipHook.getEffectiveSlot(i) === slot).length + (_pendingSlotCounts[slot] || 0);
            const capacity = this.system?.slotCapacities?.[slot] ?? 1;
            obj.system.equipped = used < capacity;
            if (obj.system.equipped) _pendingSlotCounts[slot] = (_pendingSlotCounts[slot] || 0) + 1;
          } else {
      ...
      if (obj.system?.creationChanges && obj.system.creationChanges.length) {
        // D35E-CN: 创建时效果公式支持 @角色属性
        const creationRollData = this.getRollData(null, true);
        for (let creationChange of obj.system.creationChanges) {
          if (creationChange && creationChange[0] && creationChange[1] != null &&
              String(creationChange[1]).trim() !== '') {
            creationRollData.item = obj.system;
            let total = 0;
            try {
              total = new Roll35e(String(creationChange[1]), creationRollData).roll().total;
            } catch (err) {
              ui.notifications.warn(
                  `创建时效果公式无效："${creationChange[1]}"（${err.message || err}）`);
            }
            setProperty(obj.system, creationChange[0], total);
          }
        }
  ...
  async _updateMinions(options = {}) {
    ...
    // 0) 主控自己身上若有随从职业，也同步
    updates.push(this._syncMinionClass(master, minionLevels)
    ...
    for (const minion of game.actors) {
      ...
      }, {stopUpdates: true}).catch((e) => LogHelper.error('Minion master sync failed', e)));

      // 2) 同步随从职业等级
      updates.push(this._syncMinionClass(minion, minionLevels)
          .catch((e) => LogHelper.error('Minion class sync failed', e)));
    }
    await Promise.all(updates);
  }

  async _syncMinionClass(actor, minionLevels) {
    const minionClass = actor.items.find(
        (o) => getProperty(o.system, 'classType') === 'minion');
    if (!minionClass) {
      LogHelper.log('SYNC-DEBUG: minion class NOT FOUND on', actor.name);
      return;
    }
    const group = (minionClass.system.minionGroup || '').toLowerCase();
    const level = minionLevels[group] ?? 0;
    LogHelper.log('SYNC-DEBUG:', {
      actor: actor.name,
      item: minionClass.name,
      group: group,
      currentLevels: minionClass.system.levels,
      targetLevel: level,
