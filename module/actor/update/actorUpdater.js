...
export class ActorUpdater {
  ...
  async #prepareUpdateData(updated) {
    ...
    // [D35E-CN]已移除：死亡时给 token 加骷髅叠加效果（overlayEffect=skull.svg，用户要求去掉死亡骷髅标记）
    ...
    for (let i of this.actor.items.values()) {
      ...
      // 禁止容器嵌套：容器物品不能放入另一个容器（防循环依赖）
      if (i.type === "loot" && i.system.subType === "container" && i.system.containerId && i.system.containerId !== "none") {
      ...
      } else if (i.system.containerId !== undefined && i.system.containerId !== "none") {
  ...
  async updateChanges({ updated = null } = {}, options = {}) {
    ...
    const changeObjects = source.items
        ...
        .filter((obj) => {
          ...
          if (obj.type === "equipment" || obj.type === "weapon")
            return this.#isItemEffective(obj) && !obj.system.melded && !obj.broken;
          return true;
        });
      ...
      if (typeof buffTarget === "object") {
        ...
        if (key === "skill") {
          ...
          for (const slotKey of Object.keys(CONFIG.D35E.defaultSlotCapacities)) {
            ...
            Object.keys(CONFIG.D35E.bonusModifiers).forEach((b) => {
              changeData[`slot.${slotKey}`][b] = duplicate(changeDataTemplate);
            });
          }
        }
        // Add static targets
    ...
    allChanges.forEach((change, a) => {
      ...
      if (currentChangeTarget !== changeTarget) {
        ...
        // D35E-CN: 注入角色物品名单供变化公式条件判断，如 ${this.hasItem('魔法飞弹法杖') ? 2 : 0}
        changeRollData.items = (this.actor.items || []).map((i) => i.name);
        changeRollData.hasItem = (name) => changeRollData.items.includes(name);
  ...
  async #resetData(updateData, source, flags, sourceInfo, changes, fullConditions) {
    ...
    // [D35E-CN修复] 触及变化加成必须在应用被动变化前重置，否则每次更新重复叠加
    // （物品提供 +5 触及 → reachBonus 每次 update 都 +5：10、15、20…；删除物品也不降）
    linkData(source, updateData, "system.traits.reachBonus", 0);
    ...
    items
        .filter((obj) => {
          return obj.type === "equipment" && this.#isItemEffective(obj) && !obj.system.melded && !obj.broken;
        })
        .forEach((obj) => {
    ...
    // Total sneak attak dice
    {
      ...
      classes.forEach((obj) => {
        try {
          ...
          if (!groupFormulas.has(obj.system.sneakAttackGroup) && obj.system.sneakAttackFormula) {
      ...
      for (var key of groupLevels.keys()) {
        const formula = groupFormulas.get(key);
        if (!formula) continue;
        const v = new Roll35e(formula, { level: groupLevels.get(key) }).roll().total;
    ...
    // Total sneak attak dice
    {
      ...
      classes.forEach((obj) => {
        try {
          // 保护：随从职业不参与自身 minionClassLevels 的计算，防止"左脚踩右脚"循环叠加
          if (getProperty(obj.system, "classType") === "minion") return;
          ...
          if (!groupFormulas.has(minionGroup) && obj.system.minionLevelFormula) {
  ...
  async #addDefaultChanges(source, changes, flags, sourceInfo, fullConditions, sizeOverride, options = {}, updateData) {
    ...
    if (!options.skipToken && this.actor.isCharacterType) {
      ...
      for (let i of this.actor.items.values()) {
        ...
        if (this.#isItemEffective(i) && !i.system.melded && !i.broken) {
   ...
   * 物品效果是否对角色生效：
   * - 装备/武器 equipped，或
   * - 位于容器内且打开了“在角色身上生效”（system.containerActive，不占装备槽位）
   */
  #isItemEffective(obj) {
    if (
      obj.system?.containerActive === true &&
      obj.system?.containerId &&
      obj.system?.containerId !== "none"
    )
      return true;
    return obj.system?.equipped === true;
  }
