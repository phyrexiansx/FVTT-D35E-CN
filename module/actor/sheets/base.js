...
import { ContainerApplication } from "../../item/container-sheet.js";
...
export class ActorSheetPF extends ActorSheet {
  constructor(...args) {
    ...
    // [优化] 攻击/伤害/射程描述缓存：数据未变化时避免重复掷骰求值
    this._itemDescriptionCache = new Map();
    this._onActorDataChanged = (doc) => {
      if (doc === this.actor || doc?.parent === this.actor) this._itemDescriptionCache.clear();
    };
    for (const h of ["updateActor", "deleteActor", "createItem", "updateItem", "deleteItem"]) {
      Hooks.on(h, this._onActorDataChanged);
    }
  ...
  async getData() {
    ...
    sheetData.items = sheetData.actor.items.map((i) => {
      ...
      let desc = this._itemDescriptionCache.get(item.id);
      if (!desc) {
        desc = {
          attack: item.type === "attack" ? ItemDescriptionsHelper.attackDescription(item, actorRollData) : "",
          damage: item.type === "attack" ? ItemDescriptionsHelper.damageDescription(item, actorRollData) : "",
          range: item.type === "attack" ? ItemDescriptionsHelper.rangeDescription(item) : "",
        };
        this._itemDescriptionCache.set(item.id, desc);
      }
      i.attackDescription = desc.attack;
      i.damageDescription = desc.damage;
      i.range = desc.range;
  ...
  async activateListeners(html) {
    ...
    html.find(".open-container").click((ev) => this._onOpenContainer(ev));
    html.find(".intuitive-toggle").change(async (ev) => {
  ...
  async _onItemChangeContainer(event) {
    ...
    if (!item) return;
    // 禁止容器嵌套：容器不能放入另一个容器
    if (newSource && newSource !== "none" && newSource !== "") {
      if (item.type === "loot" && item.system.subType === "container") {
        ui.notifications.warn(game.i18n.localize("D35E.ContainerNestingForbidden"));
        $(event.currentTarget).val(item.system.containerId || "");
        return;
      }
      const target = this.actor.getOwnedItem(newSource);
      if (!target) return;
      item.update({
        "system.containerId": newSource,
        "system.container": target.name,
        "system.containerWeightless": !!target.system.bagOfHoldingLike,
  ...
  /** 以独立窗口打开容器 */
  async _onOpenContainer(event) {
    event.preventDefault();
    const itemId = event.currentTarget.closest(".item").dataset.itemId;
    const item = this.actor.getOwnedItem(itemId);
    if (!item) return;
    new ContainerApplication(item, this.actor).render(true);
  }

  async _onCharacterClearContainers(event) {
    event.preventDefault();
    let itemUpdates = [];
    // 解除所有容器关联：容器内物品的 containerId 清空，使其回到角色列表
  ...
  async _quickEquipItem(event) {
    ...
    if (!hasProperty(item.system, "equipped")) return;
  ...
  async _equipItemInSlot(item, slotKey) {
    ...
    const choiceIndex = await new Promise((resolve) => {
      new Dialog({
        title: game.i18n.localize("D35E.EquipChooseSlot"),
        content: `<p>${game.i18n.format("D35E.EquipChooseSlotHint", { name: item.name })}</p>${choiceHtml}`,
        buttons: {
          ok: {
            label: game.i18n.localize("D35E.Confirm"),
            callback: (html) => {
              const sel = html.find('input[name="slotChoice"]:checked');
              resolve(sel.length ? parseInt(sel.val()) : 0);
            },
          },
          cancel: {
  ...
  async _onItemEdit(event) {
    ...
    try {
      ...
      console.error("D35E | Actor refresh failed before item edit:", e);
    }
  ...
  _prepareItems(sheetData) {
    ...
    for (let i of items) {
      ...
      // 容器内“在角色身上生效”的武器视为可用（不占装备槽位）
      if (
        i.type === "weapon" &&
        !i.system.melded &&
        i.system.containerId &&
        i.system.containerId !== "none" &&
        i.system.containerActive === true
      )
        equippedWeapons.add(i.id);
  ...
  async _onDropItem(event, data) {
    ...
    if (slotEl) {
      ...
      const itemId = itemIdFromUuid ?? dropData?.id ?? dropData?.data?._id ?? dropData?.data?.id;
  ...
  async addItemFromDropData(dropData) {
    ...
    // Case 2 - Data explicitly provided
      ...
      const _isItemDoc = (d) => !!d && typeof d.toObject === "function" && !!d.uuid && d.documentName === "Item";
      const sourceItem = _isItemDoc(dropData) ? dropData : (_isItemDoc(dropData?.data) ? dropData.data : null);
      if (!sourceItem) { console.warn("D35E | 拖放数据解析失败", { ctor: dropData?.constructor?.name, uuid: dropData?.uuid, dataCtor: dropData?.data?.constructor?.name }); return false; }
      ...
      if (sourceActor?.sheet?.id) {
        if (sourceActor.sheet.id.indexOf("ActorSheetPFNPCLoot") !== -1) {
          if (sourceActor.getFlag("D35E", "lootsheettype") === "loot")
            return LootSheetActions.lootItem(this.actor, sourceActor, this.actor, sourceItem.id, null);
        }
      }

      dataType = "data";
      itemData = sourceItem.toObject(false);
      // 整包复制：拖入的是容器时，收集其内容物随容器一起复制（嵌入 itemData，避免实例状态残留）
      if (itemData.type === "loot" && itemData.system?.subType === "container") {
        if (sourceActor?.items) {
          itemData._containerContents = sourceActor.items
            .filter((i) => i.system.containerId === sourceItem.id)
            .map((i) => i.toObject(false));
          console.log(`D35E | 容器整包复制: 容器「${sourceItem.name}」id=${sourceItem.id}, 收集内容物 ${itemData._containerContents.length} 件`);
        } else {
          console.warn("D35E | 容器整包复制: 未找到源角色", { hasSourceItem: !!sourceItem, hasSourceActor: !!sourceActor });
        }
      }
    }
      ...
      return new Dialog(
        {
          ...
          buttons: {
            confirm: {
              ...
              callback: (html) => {
                if (getProperty(itemData, "data.uniqueId")) delete itemData.data.uniqueId; else if (itemData.system?.uniqueId) delete itemData.system.uniqueId;
  ...
  async importItem(itemData, dataType) {
    ...
    if (itemData.type === "loot" && itemData.system?.subType === "container") {
      return this._importContainerBundle(itemData, dataType);
    }
  ...
  /**
   * 整包复制容器：先创建容器，再创建内容物并重新关联到新容器。
   * 禁止嵌套：内容物若是容器则不挂入。
   * @private
   */
  async _importContainerBundle(containerData, dataType) {
    const pending = containerData._containerContents || [];
    delete containerData._containerContents;
    if (containerData._id) delete containerData._id;
    containerData.system = containerData.system || {};
    containerData.system.containerId = "none";
    containerData.system.container = "None";
    containerData.system.equipped = false;

    const created = await this.actor.createEmbeddedEntity("Item", containerData, { dataType });
    const createdContainer = Array.isArray(created) ? created[0] : created;
      console.log(`D35E | 容器整包复制: 容器「${containerData.name}」已创建 id=${createdContainer?.id}, 待创建内容物 ${pending.length} 件`);
    if (!createdContainer || pending.length === 0) {
      this.render(false);
      return createdContainer || created;
    }

    const newId = createdContainer.id;
    const contentData = pending.map((c) => {
      if (c._id) delete c._id;
      c.system = c.system || {};
      if (c.type === "loot" && c.system.subType === "container") {
        c.system.containerId = "none";
        c.system.container = "None";
        return c;
      }
      c.system.containerId = newId;
      c.system.container = containerData.name;
      c.system.containerWeightless = !!containerData.system.bagOfHoldingLike;
      // 容器内物品不允许占用装备槽位
      c.system.equipped = false;
      return c;
    });
    const createdContents = await this.actor.createEmbeddedEntity("Item", contentData, { dataType: "data" });
      console.log(`D35E | 容器整包复制: 内容物创建 ${createdContents?.length ?? 0}/${contentData.length} 件`);
    this.render(false);
