    ...
    static _preProcessDiceFormula(formula, data = {}) {
        function _fillTemplate(templateString, templateVars){
            if (templateString.indexOf('$') !== -1)
                try {
                    // D35E-CN: 模板插值内支持 @属性 引用（@item.level -> ${this.item.level}，缺失按 0 处理）
                    // 注：必须用 ${...} 模板插值形式，让 new Function 在 templateVars 上下文立即求值；
                    // 若替换为字面 (this.x ?? 0) 会交给 Roll.safeEval（this=Roll 实例）求值失败。
                    const ts = templateString.replace(/@([a-z.0-9_\-]+)/gi, (match, term) => "\${this." + term + " ?? 0}");
                    return new Function("return `" + ts + "`;").call(templateVars);
                } catch (err) {
                    ui.notifications.warn(game.i18n.format("DICE.WarnMissingData", {match: templateString}));
                    return  "0";
                }
            else
                return formula;
