  ...
  prepareSheetData(sheetData) {
    ...
    if (this.sheet.item.system.changes) {
      ...
      sheetData.data.system.changes.forEach((item) => {
        ...
        // Add equipment slot subtargets (slot.armor / slot.head / slot.ring ...)
        else if (item[1] === "slot") {
          for (let [k, v] of Object.entries(CONFIG.D35E.equipmentSlots)) {
            for (let [s, label] of Object.entries(v)) {
              item.subTargets[`slot.${s}`] = label;
            }
          }
        }
        // Add static targets
