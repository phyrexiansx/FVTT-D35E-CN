...
import { ContainerApplication } from "../container-sheet.js";
...
export class ItemSheetPF extends ItemSheet {
  ...
  async getData() {
    ...
    // 绑定光环（aura.html）：候选目标列表 + 是否绑定模式（仅源光环显示 UI，复制品隐藏）
    sheetData.isAura = this.item.type === "aura";
    sheetData.isAuraSource = this.item.type === "aura" && !getProperty(this.item.system, "sourceAuraId");
    sheetData.isLinkedAura =
      this.item.type === "aura" &&
      !getProperty(this.item.system, "sourceAuraId") &&
      getProperty(this.item.system, "auraMode") === "linked";
    if (this.item.type === "aura") {
      ...
      for (const token of (canvas?.scene?.tokens?.values ? canvas.scene.tokens.values() : [])) {
        if (!token.actor || !token.actor.id) continue;
        const linked = !!token.actorLink || !!token.document?.actorLink;
        if (!opts.has(token.actor.id)) {
          opts.set(token.actor.id, {
            value: `token:${token.id}`,
            actorId: token.actor.id,
            tokenId: linked ? "" : token.id,
            name: `${token.name}（Token${linked ? " · Linked" : ""}）`,
          });
        }
      }
      sheetData.linkedTargetOptions = [...opts.values()];
    }
    ...
    if (this.item.type === "race") {
      ...
      // 注意：此处原本会串行加载整个 D35E.spelllike 宝典用于填充 children.spelllikes，
      // 但没有任何模板渲染 children.spelllikes（死代码），且类法术能力已由 buildCache 并入
      // CACHE.RacialFeatures（在下方的 abilities 列表中显示），故移除以避免打开种族物品卡顿。
      {
      ...
      for (let ability of this.item.system.addedAbilities || []) {
        ...
        if (!e) continue; // 旧数据引用的能力不在缓存中（未加载的宝典/已改名/已删除），跳过
   ...
   * @private
   */
  async _onOpenContainerItem(event) {
    event.preventDefault();
    if (!this.item.actor) return;
    new ContainerApplication(this.item, this.item.actor).render(true);
  }
  ...
  activateListeners(html) {
    ...
    // 打开容器独立窗口（loot 卡上的按钮）
    html.find(".open-container-item").click((ev) => this._onOpenContainerItem(ev));
    ...
    html.find(".linked-target-control").click(this._onLinkedTargetControl.bind(this));
  ...
  /**
   * 绑定光环：添加 / 移除指定目标（aura.html）
   * @private
   */
  async _onLinkedTargetControl(event) {
    event.preventDefault();
    const a = event.currentTarget;
    await this._onSubmit(event);

    if (a.classList.contains("add-linked-target")) {
      const select = this.form.querySelector("select.linked-target-select");
      if (!select || !select.selectedOptions || !select.selectedOptions.length) return;
      const opt = select.selectedOptions[0];
      const entry = {
        actorId: opt.dataset.actorId || "",
        tokenId: opt.dataset.tokenId || "",
        name: opt.textContent.trim(),
      };
      if (!entry.actorId) return;
      const targets = duplicate(this.item.system.linkedTargets || []);
      if (targets.some((t) => t.actorId === entry.actorId)) return;
      targets.push(entry);
      return this.item.update({ "system.linkedTargets": targets });
    }

    if (a.classList.contains("delete-linked-target")) {
      const li = a.closest(".linked-target");
      if (!li) return;
      const targets = duplicate(this.item.system.linkedTargets || []);
      targets.splice(Number(li.dataset.linkedTarget), 1);
      return this.item.update({ "system.linkedTargets": targets });
    }
  }
  ...
  async _onDropFullAttack(event) {
    ...
    // D35E-CN: 防空（合集条目没有 parent；合集打开的条目表单没有 actor）
