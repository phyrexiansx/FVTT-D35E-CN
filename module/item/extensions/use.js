...
import { getRollAdvantageMode } from "../../automation/intuitive.js";
import { triggerBattleAnim } from "../../battle-animation.js";
...
export class ItemUse {
  ...
  async use(tempActor, replacementId, ev, skipDialog, rollModeOverride, temporaryItem, skipChargeCheck) {
    ...
    // [D35E] 战斗动画：治疗/能力（普通使用发起）
    try {
      triggerBattleAnim(this.item, { targets: this._animTargets });
    } catch (e) {}
  ...
  async rollAttack(fullAttack, form, temporaryItem, actor, rollData, skipChargeCheck) {
    ...
    if (rollData.advantageMode === undefined) {
      ...
      rollData.advantageMode = _tagMode === 1 ? 'advantage' : (_tagMode === -1 ? 'disadvantage' : 'normal');
    }
    // [D35E] 战斗动画：结算前快照锁定目标（以点击攻击瞬间的最新锁定为准；结算过程中 game.user.targets 可能被清空）
    this._animTargets = Array.from(game.user.targets);
    ...
    // [D35E] 全力攻击：攻击自身加值/减值（设置页 bonus）——应用于该攻击的每次投掷
    if (rollData.faAttackBonus) {
      attackExtraParts.push({
        part: "@faAttackBonus",
        value: rollData.faAttackBonus,
        source: game.i18n.localize("D35E.FullAttackBonus"),
      });
      rollModifiers.push(`${game.i18n.localize("D35E.FullAttackBonus")} ${rollData.faAttackBonus}`);
    }
    // [D35E] 全力攻击：双武器模式（设置页 attackMode）——复用双武器战斗写死自动化
    if (rollData.faAttackMode && rollData.faAttackMode !== "primary" && !rollData.twoWeaponPenalty) {
      const twoWeaponMode = rollData.faAttackMode;
    ...
    if (this.item.hasAttack) {
      ...
      for (let atk of allAttacks) {
        ...
        await this.item._addCombatSpecialActionsToAttack(
          ...
          rollData,
          optionalFeatRanges,
          attackId + (rollData.faSeqOffset || 0)
        );
        // Add to list
        attacks.push(attack);
        attackId++;
  ...
  // [D35E] 全力攻击：弹出全力攻击窗口（调整 + 高级战斗行动 + 攻击序列标注），确认后按配置批量投掷
  async useFullAttack(
    {
      ev = null,
      skipDialog = false,
    ...
    let faActor = this.item.actor;
    if (actor !== null) faActor = actor;
    if (faActor && !faActor.isOwner) return ui.notifications.warn(game.i18n.localize("D35E.ErrorNoActorPermission"));

    // 收集有效攻击配置（保持 system.attacks 顺序）
    const attackConfigs = [];
    const attacks = getProperty(this.item.system, "attacks") || {};
    for (const key of Object.keys(attacks)) {
      const cfg = attacks[key];
      if (!cfg || !cfg.id) continue;
      const attackItem = faActor.items.get(cfg.id);
      if (!attackItem) continue;
      attackConfigs.push({
        key: key,
        id: cfg.id,
        name: cfg.name || attackItem.name,
        img: cfg.img || attackItem.img,
        count: Math.max(1, parseInt(cfg.count) || 1),
        bonus: cfg.bonus ?? 0,
        attackMode: cfg.attackMode || "primary",
        item: attackItem,
      });
    }
    if (!attackConfigs.length) return ui.notifications.warn(game.i18n.localize("D35E.FullAttackNoAttacks"));

    // 用第一个攻击的 rollData 作为窗口上下文（actionType 正确匹配攻击类 combat changes）
    const firstItemUse = new ItemUse(attackConfigs[0].item);
    firstItemUse.itemData = firstItemUse.item.getRollData();
    const rollData = faActor ? duplicate(faActor.getRollData(null, true)) : {};
    rollData.item = duplicate(firstItemUse.itemData);

    const weaponFeats = faActor.combatChangeItems.filter((o) =>
      ItemCombatChangesHelper.canHaveCombatChanges(o, rollData, `${firstItemUse.item.type}`)
    );
    const weaponFeatsOptional = faActor.combatChangeItems.filter((o) =>
      ItemCombatChangesHelper.canHaveCombatChanges(o, rollData, `${firstItemUse.item.type}Optional`)
    );
    const dialogData = {
      data: rollData,
      id: this.item.id,
      ...
      rollMode: rollModeOverride
        ? rollModeOverride
        : game.settings.get("D35E", `rollConfig`).rollConfig[faActor.type]?.attack ||
          game.settings.get("core", "rollMode"),
      rollModes: CONFIG.Dice.rollModes,
      attacks: attackConfigs,
      hasFeats: weaponFeats.length || weaponFeatsOptional.length,
      hasFeatsOrSummons: weaponFeats.length || weaponFeatsOptional.length,
      weaponFeats: weaponFeats,
      weaponFeatsOptional: weaponFeatsOptional,
    };
    const html = await renderTemplate("systems/D35E/templates/apps/full-attack-dialog.html", dialogData);

    // [D35E] 全力攻击：批量投掷序列（自动投掷用窗口 form；快进时 form 为空）
    // 注意：rd 必须每次投掷重建——rollAttack 内 _addCombatChangesToRollData 会对 rollData 做累加，
    // 复用同一 rd 会导致高级战斗行动加值在多次攻击间重复叠加
    const runSequence = async (dlgHtml) => {
      let seq = 0;
      for (const cfg of attackConfigs) {
        for (let i = 0; i < cfg.count; i++) {
          const itemUse = new ItemUse(cfg.item);
          itemUse.itemData = cfg.item.getRollData();
          const rd = faActor ? duplicate(faActor.getRollData(null, true)) : {};
          rd.item = duplicate(itemUse.itemData);
          rd.faAttackBonus = cfg.bonus;
          rd.faAttackMode = cfg.attackMode;
          rd.faSeqOffset = seq;
          await itemUse.rollAttack(false, dlgHtml || null, temporaryItem, faActor, rd, skipChargeCheck);
          seq++;
      ...
      if (game.combats.active) {
        const combatActor = this.item.getCombatActor(faActor);
        if (combatActor) combatActor.useFullAttackAction();
      }
    };

    // 快进（shift/右键）：跳过窗口直接自动投掷
    if (skipDialog || (ev instanceof MouseEvent && (ev.shiftKey || ev.button === 2))) {
      await runSequence(null);
      return { wasRolled: true, roll: null };
    }
    ...
    await new Promise((resolve) => {
      new Dialog(
        {
          title: `${game.i18n.localize("D35E.Use")}: ${this.item.name} - ${faActor.name}`,
          content: html,
          buttons: {
            normal: {
              ...
              callback: async (dlgHtml) => {
                wasRolled = true;
                const autoRoll = dlgHtml.find('[name="full-attack-auto-roll"]').prop("checked") !== false;
                if (autoRoll) {
                  await runSequence(dlgHtml);
                } else {
                  // 非自动投掷：逐个弹原攻击对话框（不预填全力攻击设置页的加值/减值，
                  // 每个攻击按干净的默认值由玩家自行调整）
                  let seq = 0;
                  for (const cfg of attackConfigs) {
                    const itemUse = new ItemUse(cfg.item);
                    itemUse.itemData = cfg.item.getRollData();
                    for (let i = 0; i < cfg.count; i++) {
                      await itemUse.useAttack(
                        {
                          ev: ev,
                          skipDialog: false,
                          attackType: cfg.attackMode,
                          isFullAttack: true,
                          rollModeOverride: rollModeOverride,
                          temporaryItem: temporaryItem,
                          faSeqOffset: seq,
    ...
    return { wasRolled: wasRolled, roll: null };
  }
  ...
  async useAttack(
    ...
    if (ev && ev.originalEvent) ev = ev.originalEvent;
    // [D35E] 战斗动画：入口快照锁定目标（use() 入口可能已快照，不覆盖）
    if (!this._animTargets) this._animTargets = Array.from(game.user.targets);
    let actor = this.item.actor;
    if (tempActor !== null) {
      actor = tempActor;
    ...
    // [D35E] 全力攻击：全局攻击序号（供「仅应用于首次攻击」判断）
    if (faSeqOffset != null) rollData.faSeqOffset = faSeqOffset;
    // 借机攻击标记（由借机攻击选择窗口设置）：供 Combat Changes 条件词条判断
    // 条件写法示例：`"@attackType" == "AO attack"` 或 `@aoAttack == 1`
    if (this.item?._pendingAoO) {
      rollData.aoAttack = 1;
    ...
    var autoFlank = game.settings.get("D35E", "automate-flanking-threat");
    const userTargets = Array.from(game.user.targets || []);
    if (userTargets.length === 1) {
      const targetPlaceable = userTargets[0];
      var surroundingTokens = DistanceHelper.getSurroundingTokens(targetPlaceable);

      let itemUserToken = null;
      if (actor.token) {
        itemUserToken = canvas.tokens.placeables.find((t) => t.document.id === actor.token.id);
      } else {
        itemUserToken = canvas.tokens.placeables.find((t) => t.actor && t.actor.id === actor.id);
      }
      if (itemUserToken) {
        for (let token of surroundingTokens) {
          ...
          var isTokenFlanking = DistanceHelper.isFlanking(itemUserToken, targetPlaceable, token);
          if (isTokenFlanking) {
        ...
        if (autoFlank) {
          isThreatening = isThreatening ||
            DistanceHelper.isAttackThreatening(itemUserToken, this.item, targetPlaceable);
        } else {
          isThreatening = true;
        }
      } else {
  ...
  extractFormData(
    ...
    // [D35E]优势/劣势：对话框「检定方式」radio（显式选择）覆盖默认（标签）；普通=跟随标签
    const advModeEl = form.find('[name="adv-mode"]:checked');
    if (advModeEl.length && advModeEl.val() !== 'normal') {
      rollData.advantageMode = advModeEl.val();
    }
    if (rollData.advantageMode === 'advantage') rollModifiers.push('优势');
    else if (rollData.advantageMode === 'disadvantage') rollModifiers.push('劣势');
    ...
    // 借机攻击开关：标记本次攻击为借机，并占用该角色的本回合借机次数
    if (form.find('[name="aooAttack"]').prop("checked")) {
      rollData.aoAttack = 1;
      rollData.attackType = "AO attack";
      if (!rollData.attackToggles) rollData.attackToggles = {};
      rollData.attackToggles.aoo = true;
      const aooToken = actor?.token;
      if (aooToken) {
        const aooCombatant = game.combat?.combatants.find((c) => c.tokenId === aooToken.id);
        if (aooCombatant) {
          const used = aooCombatant.getFlag("D35E", "usedAaoCount") || 0;
          aooCombatant.update({ "flags.D35E.usedAaoCount": used + 1 }).catch(() => {});
        }
      }
    }
  ...
  #_getSpellDC(_rollData) {
    ...
    spellDC.harmless = data.save?.harmless === true; // [D35E]无害：自动结算跳过法术抗力与豁免
    ...
    if (
      ...
      if (saveDC > 0 && data?.save?.type) {
        ...
        // [D35E]豁免标记（D）：仅在勾选「无害」时显示（无害 = 自动结算跳过法抗与豁免）
        if (data.save?.harmless === true) spellDC.description += "（D）";
