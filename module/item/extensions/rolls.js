...
export class ItemRolls {
  ...
  rollAttack(options = {}) {
    ...
    // [D35E-CN-ADV]支持 优势/劣势（d20 双骰取高/取低）
    const advMode = (rollData && rollData.advantageMode) || "normal";
    let roll = null;
    if (options.bonusOnly) {
      roll = new Roll35e(parts.join("+"), rollData).roll();
    } else {
      roll = new Roll35e(["1d20"].concat(parts).join("+"), rollData);
      roll = DicePF.applyAdvantage(roll, advMode);
      roll = roll.roll();
    }
    if (!options.bonusOnly) {
      if (advMode === "advantage") descriptionParts.unshift({ name: "优势", value: "2d20 取高" });
      else if (advMode === "disadvantage") descriptionParts.unshift({ name: "劣势", value: "2d20 取低" });
    }
