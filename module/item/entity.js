...
export class Item35E extends ItemBase35E {
  ...
  async getLinkDataFromData(itemData) {
    ...
    let linkedData = {};
    // D35E-CN v11 兼容：itemData 可能是 packItem.data（system DataModel，name/img/_id 在 document 上）
    // 或 packItem.toObject(false)（完整数据，无 document 属性），两种都兜底处理
    const doc = itemData.document;
    linkedData.name = itemData.name ?? doc?.name ?? "";
    linkedData.img = itemData.img ?? doc?.img ?? "";
    linkedData.itemId = itemData._id ?? doc?.id ?? itemData.id ?? "";
    linkedData.packId = itemData.pack ?? doc?.pack ?? "";
