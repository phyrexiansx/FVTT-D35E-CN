Hooks.on("renderSidebarTab", async (app, html) => {
  if (app instanceof SceneDirectory) {
    let button = $("<button class='import-dd' title='导入 DungeonDraft / DungeonFog 等编辑器生成的 VTT 地图（.dd2vtt / .df2vtt / .uvtt）'><i class='fas fa-file-import'></i> 导入 VTT 地图</button>")
...
Hooks.on("init", () => {
  game.settings.register("D35E", "ddimportImportSettings", {
    name: "VTT 地图导入默认路径",
    scope: "world",
    config: false,
    default: {
  ...
  game.settings.register("D35E", "ddimportOpenableWindows", {
    name: "可开启的窗户",
    hint: "窗户是否可以开启？注意：在 DungeonDraft 中取消勾选“阻挡光照”的传送门会作为窗户导入",
    scope: "world",
    config: true,
    type: Boolean,
    default: false
...
class DDImporter extends FormApplication {
  ...
  static get defaultOptions() {
    ...
    options.template = "systems/D35E/templates/ddimport/importer.html"
    options.classes.push("dd-importer");
    options.resizable = false;
    options.height = "auto";
    options.width = 400;
    options.minimizable = true;
    options.title = "导入 VTT 地图"
    return options;
  }
  ...
  async getData() {
    ...
    let settings = game.settings.get("D35E", "ddimportImportSettings")
  ...
  async _updateObject(event, formData) {
    try {
      ...
      game.settings.set("D35E", "ddimportImportSettings", {
        source: source,
        bucket: bucket,
        path: path,
        offset: offset,
    ...
    for (let door of ddDoors) {
      try {
        ...
        doors.push(new WallDocument({
          ...
          door: game.settings.get("D35E", "ddimportOpenableWindows") ? 1 : (door.closed ? 1 : 0), // 可开启窗户设置开启时所有传送门作为门；否则仅阻挡光照的传送门作为门
