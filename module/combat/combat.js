...
export class CombatantD35E extends Combatant {
  ...
  resetPerRoundCounters() {
    if (this.actor) {
      ...
      // [D35E] 直觉动作规则：上一轮使用过直觉动作 → 本轮不获得迅捷动作（立即动作消耗下一回合的迅捷动作）
      const usedImmediate = this.getFlag("D35E", "usedImmediateAction") === true;
      const ruleOn = game.settings.get("D35E", "immediateActionRule") !== false;
      this.setFlag("D35E", "usedSwiftAction", usedImmediate && ruleOn);
      this.setFlag("D35E", "usedImmediateAction", false);
    }
  }
  ...
  useAction(activationCost) {
    ...
    } else if (activationCost.type === "immediate") {
      // [D35E] 直觉动作：标记已用，下回合动作计数器不获得迅捷动作
      if (game.settings.get("D35E", "immediateActionRule") !== false) {
        this.setFlag("D35E", "usedImmediateAction", true);
      }
    }
  }
  ...
  get usedImmediateAction() {
    /* 直觉动作：已使用时为 true（下一回合不获得迅捷动作） */
    return this.getFlag("D35E", "usedImmediateAction") ?? false;
  }
...
export class CombatD35E extends Combat {
  ...
  /** @override 防御：创建空战斗（尚无战斗员）时 this.previous 尚未初始化，跳过回合事件管理（V11.315 核心边界缺陷） */
  ...
  async nextRound() {
    if (!game.user.isGM) {
      // [D35E] FVTT v11 的 game.socket 自定义事件无法跨客户端送达（服务器只转发白名单事件），
      // 改为 whisper+flags 通知 GM 客户端执行（见 module/combat-progress-sync.js）
      const gm =
      ...
      if (gm) {
        await ChatMessage.create({
          ...
          whisper: [gm.id],
          content: "",
          flags: { D35E: { combatProgress: { type: "round", combatId: this.id, requestId: foundry.utils.randomID(16) } } },
        });
      }
    ...
    if (!game.user.isGM) {
      ...
      if (gm) {
        await ChatMessage.create({
          ...
          whisper: [gm.id],
          content: "",
          flags: { D35E: { combatProgress: { type: "turn", combatId: this.id, requestId: foundry.utils.randomID(16) } } },
        });
      }
