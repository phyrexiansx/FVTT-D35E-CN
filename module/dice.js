...
export class DicePF {
  ...
  /**
   * [D35E-CN-ADV] 应用 优势/劣势 到尚未投掷的 d20 检定。
   * 把第一个 d20 骰子项改为投两次并保留最高(kh)或最低(kl)，
   * 与 dnd5e v11 的 D20Roll.configureModifiers 实现方式一致（就地修改骰子项，避免重解析公式）。
   * @param {Roll35e} roll    已构建但未投掷的 Roll 实例
   * @param {String} advMode  "normal" | "advantage" | "disadvantage"
   * @returns {Roll35e}
   */
  static applyAdvantage(roll, advMode) {
    if (advMode !== "advantage" && advMode !== "disadvantage") return roll;
    const d20 = roll.terms.find((t) => t instanceof Die && t.faces === 20);
    if (!d20) return roll;
    d20.number = 2;
    d20.modifiers.push(advMode === "advantage" ? "kh" : "kl");
    if (!d20.options) d20.options = {};
    d20.options.advantage = advMode === "advantage";
    d20.options.disadvantage = advMode === "disadvantage";
    // 保留原始公式显示（@引用 不变），仅把开头的 1d20 替换为 2d20kh / 2d20kl
    roll._formula = (roll._formula || roll.formula).replace(/1d20/i, advMode === "advantage" ? "2d20kh" : "2d20kl");
    return roll;
  }
  ...
  static async d20Roll({event, parts, data, template, title, speaker, flavor, takeTwenty=true, situational=true, dynamicBonuses=[],
    ...
    let _roll = async (parts, setRoll, form) => {
      ...
      for (let a = 0; a < 1 + extraRolls.length; a++) {
        ...
        // [D35E-CN-ADV]对话框「检定方式」三选一：显式选择（优势/劣势）覆盖 Alt/Ctrl/标签；普通=跟随默认
        if (form) {
          const advEl = form.find('[name="adv-mode"]:checked');
          if (advEl.length && advEl.val() !== 'normal') {
            curParts[0] = advEl.val() === 'advantage' ? '2d20kh' : '2d20kl';
          }
        }
    ...
    // [D35E]优势/劣势：对话框radio（显式）> Alt=优势 Ctrl=劣势（手动）> 变化效果标签（优势/劣势抵消）> 普通
    let _advMode = 0;
    if (event?.altKey) _advMode = 1;
    else if (event?.ctrlKey) _advMode = -1;
    else {
      const _advActor = speaker ? ChatMessage.getSpeakerActor(speaker) : null;
      _advMode = _advActor ? getRollAdvantageMode(_advActor) : 0;
    }
    const _d20Expr = _advMode === 1 ? "2d20kh" : (_advMode === -1 ? "2d20kl" : "1d20");
    // Modify the roll and handle fast-forwarding
    parts = [_d20Expr].concat(parts);
    ...
    return new Promise(resolve => {
      new Dialog({
        ...
        buttons: {
          ...
          advantage: {
            label: "优势",
            callback: html => roll = _roll(["2d20kh"].concat(parts.slice(1)), staticRoll != null ? staticRoll : -1, html)
          },
          disadvantage: {
            label: "劣势",
            callback: html => roll = _roll(["2d20kl"].concat(parts.slice(1)), staticRoll != null ? staticRoll : -1, html)
          }
        },
        default: "normal",
        close: html => {
