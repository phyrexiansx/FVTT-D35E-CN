...
export const registerSystemSettings = function() {
  ...
  /**
   * Immediate action rule: using an immediate action consumes next round's swift action
   */
  game.settings.register("D35E", "immediateActionRule", {
    name: "直觉动作规则",
    hint: "开启后：玩家使用直觉动作，其角色下一回合动作计数器不获得迅捷动作；并在战斗追踪器显示直觉动作计数。",
    scope: "world",
    config: true,
    default: true,
    type: Boolean,
  ...
  game.settings.register("D35E", '__onboarding', {
    name: `已看过教程`,
    hint: '系统已经显示过教程，如果想重新看一遍，关闭此选项并刷新。',
    scope: 'client',
    default: false,
    config: true,
    type: Boolean,
  ...
  game.settings.register("D35E", "debug-distance-overlay", {
    name: "显示距离调试覆盖层",
    hint: "绘制威胁范围边界框的调试矩形。仅供开发者使用。",
    scope: "world",
    config: true,
    requiresReload: true,
    default: false,
  ...
  game.settings.register("D35E", "threatened-display-mode", {
    name: "威胁范围显示",
    hint: "控制选中棋子时威胁范围的显示方式：无、染色受威胁棋子、或绘制完整威胁区域。",
    scope: "world",
    config: true,
    requiresReload: true,
    default: "tokens",
  ...
  game.settings.register("D35E", "automate-flanking-threat", {
    name: "自动夹击/威胁检测",
    hint: "启用后自动检测夹击并在攻击对话框中预勾选夹击加成；同时启用威胁距离检查。",
    scope: "world",
    config: true,
    requiresReload: true,
    default: false,
  ...
  game.settings.register("D35E", "advanced-combat-tracking", {
    name: "高级战斗追踪",
    hint: "启用后离开受威胁区域时会提示借机攻击；战斗追踪器显示每轮动作标记。",
    scope: "world",
    config: true,
    requiresReload: true,
    default: true,
  ...
  game.settings.register("D35E", "autoApplyIntuitive", {
    name: "SETTINGS.D35EAutoApplyIntuitiveName",
    hint: "SETTINGS.D35EAutoApplyIntuitiveHint",
    scope: "world",
    config: true,
    type: Boolean,
    default: true
