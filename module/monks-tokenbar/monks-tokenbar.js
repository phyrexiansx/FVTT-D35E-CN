...
export class MonksTokenBar {
    ...
    static init() {
        ...
        MonksTokenBar.SOCKET = "message"; // [D35E] 事件名；通道由 socketlib.registerSystem("D35E") 提供
        ...
        game.keybindings.register('monks-tokenbar', 'request-roll', {
            ...
            restricted: true,
            onDown: (data) => {
                let roll = game.user.getFlag("D35E", "lastmodeST");
        ...
        patchFunc("ChatLog.prototype._getEntryContextOptions", function (wrapped, ...args) {
            ...
            let canHeroPointReroll = ($li) => {
                ...
                if (!message.getFlag("D35E", "what"))
                    return false;

                let msgToken = message.getFlag("D35E", `token${MonksTokenBar.contextId}`);
                if (!msgToken)
                    return false;
            ...
            return [...menu, ...[
                {
                    ...
                    callback: $li => {
                        const message = game.messages.get($li[0].dataset.messageId, { strict: !0 });
                        let what = message.getFlag("D35E", "what");
                        if (what == "savingthrow")
    ...
    static ready() {
        // [D35E]socketlib system socket 会在两端触发 unknown type 报错 → 改 ChatMessage whisper + flags 通道
        Hooks.on("createChatMessage", (message) => {
            const data = message.getFlag("D35E", "mtbSocket");
            if (!data) return;
            if (message.user.id === game.user.id) { //自己发出的消息：不处理（GM 负责清理）
                if (game.user.isGM) setTimeout(() => message.delete().catch(() => {}), 1000);
                return;
            }
            MonksTokenBar.onMessage(data);
            if (game.user.isGM) setTimeout(() => message.delete().catch(() => {}), 1000); //GM 延迟清理（聊天一闪而过的空白 whisper 属预期）
        });
        // [D35E]隐藏快速消失的耳语（跨用户通信消息不显示在聊天里，渲染后由 GM 删除）
        Hooks.on("renderChatMessage", (message, html) => {
            if (message.getFlag("D35E", "mtbSocket")) html.hide();
        });
            ...
            if (npcObject != undefined) {
                ...
                npcSheetNames.forEach((sheetName) => {
                    Hooks.on("render" + sheetName, (app, html, data) => {
                        ...
                        if (app?.token?.actor?.getFlag("D35E", 'converted') && app.element.find(".revert-lootable").length == 0) {
    ...
    static emit(action, args = {}) {
        ...
        // [D35E]FVTT v11 系统不可用 socket（module.* 被丢弃 / system.* 报 unknown type，见维护总结踩坑）
        // 跨用户通信改走 ChatMessage whisper + flags 通道（接收端 createChatMessage hook 处理，GM 延迟删除）
        const recipients = game.users.filter(u => u.active && !u.isSelf).map(u => u.id);
        ChatMessage.create({
            content: "",
            whisper: recipients,
            type: CONST.CHAT_MESSAGE_TYPES.OOC,
            flags: { "D35E": { mtbSocket: args } }
        });
    }
        ...
        switch (data.action) {
            ...
            case 'setRolls': {
                if (game.user.isGM) {
                    ...
                    if (msg) {
                        let rolls = duplicate(msg.getFlag("D35E", "rolls") || {});
                        rolls[data.tokenid] = data.roll;
                        await msg.setFlag("D35E", "rolls", rolls);
    ...
    static async changeGlobalMovement(movement) {
        ...
        if (MonksTokenBar.tokenbar != undefined) {
            ...
            for (let i = 0; i < tokenbar.entries.length; i++) {
                await tokenbar.entries[i].token?.setFlag("D35E", "movement", null);
    ...
    static async changeTokenMovement(movement, tokens) {
        ...
        for (let token of tokens) {
            ...
            let oldMove = token.getFlag("D35E", "movement");
            if (newMove != oldMove) {
                await token.setFlag("D35E", "movement", newMove);
                delete token._movementNotified;

                let dispMove = token.getFlag("D35E", "movement") || game.settings.get("monks-tokenbar", "movement") || MTB_MOVEMENT_TYPE.FREE;
    ...
    static async changeTokenPanning(tokens) {
        ...
        for (let token of tokens) {
            let oldPanning = token.getFlag("D35E", "nopanning");
            await token.setFlag("D35E", "nopanning", !oldPanning);
        }
    }
    ...
    static allowMovement(token, notify = true) {
        ...
        if (!game.user.isGM && token != undefined) {
            let movement = token.getFlag("D35E", "movement") || game.settings.get("monks-tokenbar", "movement") || MTB_MOVEMENT_TYPE.FREE;
            if (setting('debug'))
    ...
    static selectActors(message, filter, event) {
        let tokens = Object.entries(message.flags["D35E"])
    ...
    static onClickMessage(message, html) {
        if (MonksTokenBar.grabmessage != undefined) {
            ...
            if (message.rolls.length) {
                ...
                let msgtoken = MonksTokenBar.grabmessage.getFlag("D35E", 'token' + tokenId);

                if (msgtoken != undefined) {
                    if (MonksTokenBar.grabmessage.getFlag("D35E", 'what') == 'contestedroll')
    ...
    static toggleMovement(combatant, event) {
        ...
        let movement = (combatant.token.getFlag("D35E", 'movement') == MTB_MOVEMENT_TYPE.FREE ? MTB_MOVEMENT_TYPE.COMBAT : MTB_MOVEMENT_TYPE.FREE);
        ...
        $('.combatant', html).each(function () {
            ...
            if (combatant && combatant.hasPlayerOwner && combatant.token) {
                $($('<a>')
                    ...
                    .toggleClass('active', combatant.token.getFlag("D35E", 'movement') == MTB_MOVEMENT_TYPE.FREE)
...
Hooks.on("renderTokenConfig", (app, html, data) => {
    if (game.user.isGM) {
        let include = app.token.getFlag("D35E", 'include') || 'default';
        ...
        let group = $('<div>')
            ...
            .append($('<select>').attr('name', 'flags.D35E.include')
...
Hooks.on("renderChatMessage", async (message, html, data) => {
    ...
    if (levelCard.length !== 0) {
        let actor = await fromUuid(message.getFlag("D35E", "actor"));
        let level = parseInt(message.getFlag("D35E", "level"));
...
Hooks.on("preUpdateChatMessage", (message, data, dif, userId) => {
    if (game.user.isGM && data.whisper != undefined) {
        ...
        if (data.whisper.length == 0 && rollmode != "roll") {
            setProperty(data, "flags.D35E.oldroll", rollmode);
            setProperty(data, "flags.D35E.rollmode", "roll");
        } else if (data.whisper.length && rollmode == "roll") {
            let oldroll = message.getFlag("D35E", 'oldrollmode') || "gmroll";
            setProperty(data, "flags.D35E.rollmode", oldroll);
        }
    }
});
