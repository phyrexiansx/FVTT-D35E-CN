...
export class SavingThrowApp extends Application {
    constructor(entries, options = {}) {
        ...
        if (this.entries.length == 0) {   //if none have been selected then default to the party
            let tokens = canvas.tokens.placeables.filter(t => {
                let include = t.document.getFlag("D35E", 'include');
        ...
        this.rollmode = (options?.rollmode || options?.rollMode || game.user.getFlag("D35E", "lastmodeST") || 'roll');
        ...
        this.autopopup = game.user.getFlag("D35E", "mtbAutoPopup") ?? (options.autopopup || false); // [D35E]记忆开关状态
    }
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            template: "./systems/D35E/module/monks-tokenbar/templates/savingthrow.html",
            width: 800,
            popOut: true
        });
    }
    ...
    async requestRoll(roll) {
        ...
        if (this.entries.length > 0) {
            ...
            game.user.setFlag("D35E", "lastmodeST", rollmode);
            ...
            game.user.setFlag("D35E", "lastmodeHideNPCName", this.hidenpcname);
            let requestdata = {
            ...
            const html = await renderTemplate("./systems/D35E/module/monks-tokenbar/templates/svgthrowchatmsg.html", requestdata);
            ...
            //chatData.flags["D35E"] = {"testmsg":"testing"};
            setProperty(chatData, "flags.D35E", requestdata);
            ...
            if (this['active-tiles'])
                msg.setFlag("D35E", 'active-tiles', this['active-tiles']);
    ...
    activateListeners(html) {
        ...
        $('#monks-tokenbar-autopopup', html).click($.proxy(function (e) {
            this.autopopup = $(e.currentTarget).prop('checked');
            game.user.setFlag("D35E", "mtbAutoPopup", this.autopopup).catch(() => {}); // [D35E]记忆开关状态
        }, this));
...
export class SavingThrow {
    ...
    static async returnRoll (id, roll, actor, rollmode, msgId) {
        ...
        if (roll != undefined) {
            if (roll instanceof Combat) {
                ...
                if (message) {
                    let rolls = getProperty(message, "flags.D35E.rolls");
                    realroll = rolls[id];
                    if (realroll)
    ...
    static async onRollAbility(ids, message, fastForward, evt) {
        ...
        let flags = message.flags["D35E"];

        let requests = message.getFlag("D35E", 'requests');
        let rollmode = message.getFlag("D35E", 'rollmode');
    ...
    static async updateMessage(updates, message, reveal = true) {
        ...
        for (let update of updates) {
            if (update != undefined) {
                let msgtoken = duplicate(message.getFlag("D35E", 'token' + update.id));
                ...
                if (update.roll) {
                    ...
                    if ($.isNumeric(dc))
                        // [D35E]防御：update.roll 存在但非 Roll 实例时尝试重建（可能经序列化），并防止 msgtoken.roll 为空时 rollSuccess 崩
                        if (update.roll && !(update.roll instanceof Roll)) {
                            console.warn('[D35E-TokenBar] update.roll 非Roll实例', update.roll);
                            try {
                                if (typeof update.roll === 'string') update.roll = Roll.fromJSON(update.roll);
                                else if (update.roll.formula) update.roll = Roll.fromData(update.roll);
                            } catch (e) { }
                        }
                        if (msgtoken.roll == undefined && update.roll) console.warn('[D35E-TokenBar] msgtoken.roll 为空', update.id, update.roll);
                        if (update.roll && !(update.roll instanceof Roll)) {
                            try {
                                if (typeof update.roll === 'string') update.roll = Roll.fromJSON(update.roll);
                                else if (update.roll.formula) update.roll = Roll.fromData(update.roll);
                    ...
                    //await message.setFlag("D35E", 'token' + update.id, msgtoken);
        ...
        if (game.system.id == 'dnd5e') {
            let rolls = [];
            for (let key of Object.keys(getProperty(message, "flags.D35E"))) {
                if (key.startsWith('token')) {
                    let token = flags[key] || message.flags["D35E"][key];
                    if (token.roll) {
        ...
        await message.update({ content: content[0].outerHTML, flags: { 'D35E': flags } });
        ...
        let failed = 0;
        let passed = 0;
        let tokenresults = Object.entries(message.flags["D35E"])
            .filter(([k, v]) => {
        ...
        if (passed + failed == tokenresults.length) {
            ...
            if (message.getFlag("D35E", 'active-tiles')) {
                let restart = message.getFlag("D35E", 'active-tiles');
                let tile = await fromUuid(restart.tile);
            ...
            if (message.mtb_callback)
                message.mtb_callback.call(message, result, message.getFlag("D35E", 'options'));
    ...
    static async finishRolling(updates, message, event) {
        ...
        if (!game.user.isGM) {
            ...
            message.update({ flags: { 'D35E': flags } });
        }
    }
    ...
    static async setRollSuccess(tokenid, message, success, event) {
        if (event?.stopPropagation) event.stopPropagation();
        if (event?.preventDefault) event.preventDefault();
        //let actors = JSON.parse(JSON.stringify(message.getFlag("D35E", 'actors')));
        let msgtoken = duplicate(message.getFlag("D35E", 'token' + tokenid)); //actors.find(a => { return a.id == actorid; });
        ...
        await message.setFlag("D35E", 'token' + tokenid, msgtoken);
    }
    ...
    static async onAssignDeathST(tokenId, message, e) {
        if (game.user.isGM) {
            let msgtoken = message.getFlag("D35E", 'token' + tokenId);
    ...
    static async rerollFromMessage (message, tokenid, { heroPoint = !1, keep = "new" } = {}) {
        ...
        const rollmode = message.getFlag("D35E", "rollmode");
    ...
    static async updateReroll(message, tokenid, roll, { heroPoint = !1, keep = "new" } = {}) {
        ...
        message.setFlag("D35E", `token${tokenid}`, msgToken);
        let flags = {};
...
Hooks.on("renderChatMessage", async (message, html, data) => {
    ...
    if (svgCard.length !== 0) {
        ...
        //let actors = message.getFlag("D35E", 'actors');
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let msgtoken = message.getFlag("D35E", 'token' + tokenId);//actors.find(a => { return a.id == actorId; });
            if (msgtoken) {
                //let actor = game.actors.get(msgtoken.actorid);
                let tokenOrActor = await fromUuid(msgtoken.uuid);
                let actor = tokenOrActor?.actor ? tokenOrActor.actor : tokenOrActor;
                ...
                // [D35E]自动弹出玩家检定（autopopup 开关）：为所属玩家自动弹出掷骰窗口，且此过程不自动结算
                if (!game.user.isGM && message.getFlag("D35E", "autopopup") && msgtoken.roll == undefined && actor?.isOwner) {
                    if (game.user.getFlag("D35E", "autopopupMsg") !== message.id) {
                        // [D35E]玩家无权更新他人 ChatMessage：防重改用玩家自己的 user flag
                        await game.user.setFlag("D35E", "autopopupMsg", message.id);
                        new Dialog({
                            title: message.getFlag("D35E", "name"),
                            content: `<p style="margin:0 0 4px;">GM正要求你进行 [${message.getFlag("D35E", "name") || ""}]</p>${dc ? `<p style="margin:0;">DC: ${dc}</p>` : ''}`,
                            buttons: {
                                roll: { label: game.i18n.localize("MonksTokenBar.Roll"), callback: () => SavingThrow.onRollAbility(msgtoken.id, message, null) }
                            }
                        }).render(true);
                    }
                }
