...
export class TokenBar extends Application {
	constructor(options) {
        ...
        Hooks.on('updateToken', (document, data, options) => {
            if ((game.user.isGM || setting("allow-player")) && getProperty(data, "flags.D35E.include") != undefined) {
                this.refresh();
	...
	static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            id: "tokenbar-window",
            template: "./systems/D35E/module/monks-tokenbar/templates/tokenbar.html",
            popOut: false,
    ...
    async getCurrentTokens() {
        ...
        if (game.system.id == "pf2e" && setting("use-party")) {
            this.entries = game.actors.party.members.map(a => {
                ...
                if (token) {
                    return {
                        ...
                        img: null,
                        thumb: null,
                        movement: token.document.flags["D35E"]?.movement,
                        stats: {},
                        resource1: {},
                        resource2: {},
                        cssClass: ""
            ...
            this.entries = (canvas.scene?.tokens || [])
                .filter(t => {
                    let include = t.getFlag("D35E", 'include');
                    ...
                    return {
                        ...
                        img: null,
                        thumb: null,
                        movement: t.flags["D35E"]?.movement,
                        stats: {},
                        resource1: {},
                        resource2: {},
                        cssClass: ""
        ...
        //    this.entries[0].token.constructor.updateDocuments(this.entries.map(t => { return { _id: t.id, 'flags.D35E.-=notified': null } }), { parent: canvas.scene, options: { ignoreRefresh: true } })
    }
    ...
    async updateEntry(entry, refresh = true) {
        ...
        let viewstats = entry.actor?.getFlag("D35E", 'stats') || MonksTokenBar.stats;
        let diffstats = {};
        ...
        if (entry.token && entry.movement != getProperty(entry.token, "flags.D35E.movement")) {
            diff.movement = getProperty(entry.token, "flags.D35E.movement");
        }
        ...
        if (setting("show-disable-panning-option")) {
            if (entry.nopanning != entry.token.flags["D35E"]?.nopanning) {
                diff.nopanning = entry.token.flags["D35E"]?.nopanning;
            }
        } else {
            ...
            if (!isRightMB) {
                ...
                function dragElement(elmnt) {
                    ...
                    function closeDragElement() {
                        ...
                        game.user.setFlag("D35E", 'position', position);
                        this.pos = position;
                    }
                }
            }
                ...
                callback: (li) => {
                    Dialog.confirm({
                        ...
                        yes: () => {
                            ...
                            const entry = this.entries.find(t => t.token?.id === id);
                            if (entry)
                                entry.token.setFlag("D35E", "include", "exclude");
                        }
                    });
                }
            },
    ...
    _contextMenu(html) {
        ...
        context.render = function (target) {
            ...
            let movement = entry?.token?.getFlag("D35E", "movement");
            ...
            let nopanning = entry?.token?.getFlag("D35E", "nopanning");
            if (nopanning) {
                    ...
                    if (event?.originalEvent?.ctrlKey || event?.originalEvent?.metaKey) {
                        ...
                        if (entry && (entry.movement == undefined || entry.movement == "")) {
                            ...
                            for (let entry of that.entries) {
                                if (entry.movement == "free") {
                                    entry.movement = null;
                                    await entry.token.unsetFlag("D35E", "movement");
                                }
                            }
                            entry.movement = "free";
                            await entry.token.setFlag("D35E", "movement", "free");
                            that.render(true);
                        }
                    } else {
                        if (game.user.isGM || entry.actor?.testUserPermission(game.user, "OBSERVER")) {
                            ...
                            if (token) {
                                ...
                                if (token.getFlag("D35E", "nopanning"))
                                    animate = false;
