...
export class LootablesApp extends Application {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            template: "./systems/D35E/module/monks-tokenbar/templates/lootables.html",
            width: 500,
            popOut: true,
            ...
            if (lootSheet == "item-piles") {
                ...
                for (let entry of this.entries) {
                    ...
                    if (!['dnd5e.LootSheet5eNPC', 'core.a'].includes(entry.actor.data?.flags?.core?.sheetClass))
                        newActorData.flags["D35E"].oldsheetClass = entry.actor.data?.flags?.core?.sheetClass; //token.actor._getSheetClass();
                    ...
                    if (oldItems.length > 0) {
                        if (entry.actor.getFlag("D35E", 'olditems') != undefined)
                            oldItems = oldItems.concat(entry.actor.getFlag("D35E", 'olditems'));
                        newActorData.flags["D35E"].olditems = oldItems;
                    }
    ...
    static async revertLootable(app) {
        ...
        let actorData = {
            'flags': {
                'core': {
                    'sheetClass': (actor.flags["D35E"].oldsheetClass || null)
                },
                'monks-tokenbar': {
                    'converted': false
                }
        ...
        await app.token.update({
            "overlayEffect": null,
            "alpha": app.token.getFlag("D35E", 'alpha')
        });
