...
export class ContestedRollApp extends Application {
    constructor(entries, options = {}) {
        ...
        this.rollmode = options?.rollmode || options?.rollMode || (game.user.getFlag("D35E", "lastmodeCR") || 'roll');
    ...
    static get defaultOptions() {
       // let top = ($('#tokenbar').position()?.top || $('#hotbar').position()?.top || 300) - 260;
        return mergeObject(super.defaultOptions, {
            ...
            template: "./systems/D35E/module/monks-tokenbar/templates/contestedroll.html",
            width: 450,
            //top: top,
            popOut: true
        });
    ...
    async requestRoll(roll) {
        ...
        if (this.entries[0].token != undefined && this.entries[1].token != undefined) {
            ...
            game.user.setFlag("D35E", "lastmodeCR", rollmode);
            ...
            const html = await renderTemplate("./systems/D35E/module/monks-tokenbar/templates/contestedrollchatmsg.html", requestdata);
...
export class ContestedRoll {
    ...
    static async updateMessage(updates, message, reveal = true) {
        ...
        await message.update({ content: content[0].outerHTML, flags: { 'D35E': flags } }).then(() => {
    ...
    static getTokens(message) {
        let tokens = [];
        for (let [k, v] of Object.entries(message.flags["D35E"])) {
