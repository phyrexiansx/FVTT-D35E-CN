...
export class DnD5eRolls extends BaseRolls {
    ...
    static activateHooks() {
        Hooks.on("preCreateChatMessage", (message, option, userid) => {
            if (message.getFlag("D35E", 'ignore') === true) {
                let msgid = message.getFlag("D35E", 'msgid');
                if (msgid) {
                    let msg = game.messages.get(msgid);
                    if (msg) {
                        let rolls = duplicate(msg.getFlag("D35E", "rolls") || {});
                        let tokenid = message.getFlag("D35E", 'tokenid');
                        rolls[tokenid] = message.rolls[0];
                        if (msg.isOwner)
                            msg.setFlag("D35E", "rolls", rolls);
                        else
                            MonksTokenBar.emit("setRolls", { msgid, tokenid, roll: rolls[tokenid] });
                        setProperty(msg, "flags.D35E.rolls", rolls);
                    }
                }
                return false;
            } else
