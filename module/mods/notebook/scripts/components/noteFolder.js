...
export class noteFolder {
	constructor(pID = "", pOptions = {}) {
		...
		if (pOptions.sort && !isEmpty(pOptions.sort)) {
			...
			if (this.isRoot) {
				this._sort = game.user.getFlag(cFlagScope, cNoteSortFlag) || {...cdefaultSort, title : "root"};
			}
			else {
	...
	async save() {
		if (this.isRoot) {
			...
			return await game.user.setFlag(cFlagScope, cNoteSortFlag, this.sort);
		}
		else {
