...
export class basicNote {
	...
	get soundVolume() {
		return game.user.getFlag(cFlagScope, `${cNoteVolumeFlag}.${this.id}`) ?? 0;
	}
	...
	set soundVolume(pVolume) {
		...
		this._soundwait = game.user.setFlag(cFlagScope, `${cNoteVolumeFlag}.${this.id}`, vVolume);
		...
		if (vJournalPage) {
			...
			if (vJournal) {
				vJournal.createEmbeddedDocuments("JournalEntryPage", [{
					name : this.title, 
					flags : {[cFlagScope] : {from : {id : this.id, creator : game.user.id, type : this.type}}}, 
					text : vJournalPage
				}]);
			}
		}
	...
	toggleContent() {
		...
		game.user.setFlag(cFlagScope, cNoteToggleFlag, {[this.id] : this.contentState});
	}
	
	synchToggleState() {
		let vStates = game.user.getFlag(cFlagScope, cNoteToggleFlag);
