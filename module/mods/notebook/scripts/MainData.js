import {cModuleName, Translate, cFlagScope} from "./utils/utils.js";
...
export async function cleanUserData() {
	...
	let vNotes = game.user.getFlag(cFlagScope, "notes");
	...
	if (vNotes) {
		...
		for (let vID of vIDs) {
			if (vNotes[vID] == null) {
				await game.user.unsetFlag(cFlagScope, `${cNotesFlag}.${vID}`);
			}
		}
	}
};
...
class NoteManager {
	...
	static async createNewNote(pData, pContext = {}) {
		...
		let vData = {...cDefaultNote, ...pData, owner : game.user.id, moduleversion : game.system.version};
		...
		if (CONFIG[cModuleName].noteTypes[vData.type]) {
			...
			//await game.user.setFlag(cFlagScope, cNotesFlag + `.${vID}`, vData);
	...
	static viewableNotes() {
		...
		for (let vUser of Array.from(game.users)) {
			let vUserNotes = vUser.getFlag(cFlagScope, cNotesFlag);
		...
		if (pUpdate || pContext.delete) {
			...
			if (NoteManager.canEditSelf(pID)) {
				if (NoteManager.ownsNote(pID)) {
					//await game.user.setFlag(cFlagScope, cNotesFlag + `.${pID}`, pUpdate, pContext);
					game.user.update({
						flags : {
							[cFlagScope] : {
					...
					if (game.user.isGM) {
						...
						//await vOwner.setFlag(cFlagScope, cNotesFlag + `.${pID}`, pUpdate, pContext);
						vOwner.update({
							flags : {
								[cFlagScope] : {
									[cNotesFlag + `.${pID}`] : pUpdate
	...
	static async noteUpdateRequest(pData) {
		...
		if (vIsUpdater || game.user.isGM) {
			...
			if (vRequesterhasPermission) {
				if (vIsUpdater) {
					//await game.user.setFlag(cFlagScope, cNotesFlag + `.${pData.pNoteID}`, pData.pUpdate, pData.pContext);
					game.user.update({
						flags : {
							[cFlagScope] : {
					...
					if (game.user.isGM) {
						...
						if (!vOwner?.active) {
							//await vOwner.setFlag(cFlagScope, cNotesFlag + `.${pID}`, pData.pUpdate, pData.pContext);
							vOwner.update({
								flags : {
									[cFlagScope] : {
										[cNotesFlag + `.${pData.pNoteID}`] : pData.pUpdate
	...
	static _refresh() {
		// [D35E] 数据变更后直接重渲染侧边栏（hook 链偶发不刷新，此处兜底）
		setTimeout(() => { try { ui.notebook?.render(); } catch (e) {} }, 60);
	}
	...
	static getNote(pID, pTestPermission = false) {
		if (!pTestPermission || NoteManager.canSeeSelf(pID)) {
			...
			while (i < vUsers.length && !vNote) {
				...
				vNote = vUser.getFlag(cFlagScope, cNotesFlag + `.${pID}`);
	...
	static owner(pNote) {
		...
		if (game.user.getFlag(cFlagScope, cNotesFlag + `.${pNote}`)?.owner == game.user.id) {
			return game.user;
		}
	...
	static ownsNote(pNote) {
		if (typeof pNote == "string") {
			return game.user.getFlag(cFlagScope, cNotesFlag + `.${pNote}`)?.owner == game.user.id;
		}
	...
	static permissionOverview(pNote) {
		...
		for (let vKey of Array.from(game.users.keys()).filter(vKey => (vKey != pNote.owner) && !game.users.get(vKey).isGM)) {
			let vPermission = pNote.permissions ? (pNote.permissions[vKey] || pNote.permissions.default) : undefined;
...
Hooks.on("updateUser", (pUser, pChanges, pContext) => {
	// [D35E] 注意：pChanges.flags.D35E 为展开的嵌套结构（notes: {id: data}）
	const vNoteUpdates = pChanges.flags?.[cFlagScope]?.[cNotesFlag];
	if (!vNoteUpdates) return;
	for (const vKey of Object.keys(vNoteUpdates)) {
		const vFull = pUser.flags?.[cFlagScope]?.[cNotesFlag]?.[vKey];
		const vPermission = vFull?.permissions ? Boolean(vFull.permissions[game.user.id]) : undefined;
		const vDeletion = !vNoteUpdates[vKey];

		Hooks.call(cModuleName + ".updateNote", {...vFull, id : vKey}, {...vNoteUpdates[vKey]}, {...pContext, permission : vPermission, deletion : vDeletion});
	}
});
...
Hooks.once("init", () => {
	...
	game[cModuleName] = { NoteManager: NoteManager }
