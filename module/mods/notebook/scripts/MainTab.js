import {cModuleName, cTickInterval, Translate, cFlagScope} from "./utils/utils.js";
...
//Hooks.on("renderSidebarTab", async (pDirectory, pElements, pContext) => {
	...
	} catch (e) { console.error("[Notebook] 侧边栏初始化失败:", e); }
});
...
class notesTab /*extends SidebarTab*/ {
	...
	/*
	get sortorder() {
		let vSort = game.user.getFlag(cFlagScope, cNoteSortFlag);
	...
	set sortorder(pSort) {
		...
		game.user.setFlag(cFlagScope, cNoteSortFlag, vSort);
	}
	...
	*/
	...
	render() {
		// [D35E] 重新读取笔记数据（确保渲染包含最新创建/删除），再清空重建
		this.notes = NoteManager.viewableNotes();
		if (this.tab) { this.tab.innerHTML = ""; }
		...
		vNewNoteButton.onclick = (pEvent, pContext = {}) => {
			...
			if (vQuickType && vQuickType !== "manually") {
	...
	*/
	...
	renderUpdate(pNewNoteData, pNoteDataUpdate, pContext) {
		...
		// [D35E] 兜底：50ms 后整体重渲染，确保创建/删除/跨用户笔记立即显示（局部 addNote 链路偶发不刷新）
		if (!this._nbRenderTimer) {
			this._nbRenderTimer = setTimeout(() => {
				this._nbRenderTimer = null;
				try { this.render(); } catch (e) { console.error("[Notebook] 重渲染失败:", e); }
			}, 50);
		}
	}
