const log = (...args) => console.log("D35E|update-images|", ...args);
const error = (...args) => console.error("D35E|update-images|", ...args);
...
export class UpdateImages extends FormApplication {
    ...
    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
            ...
            classes: ["form"],
            title: "Update Images",
            template: "systems/D35E/module/mods/monks-little-details/templates/update-images.html",
    ...
    async getData() {   //this will get overwritten by Hidden Initiative if it's installed.
        ...
        data.compendium = game.user.getFlag("D35E", "compendium");

        for (let i = 1; i <= 3; i++) {
            data["avatar-folder" + i] = game.user.getFlag("D35E", "avatar" + i);
            data["token-folder" + i] = game.user.getFlag("D35E", "token" + i);
        }
        data["sound-folder"] = game.user.getFlag("D35E", "sound-folder");
    ...
    convert() {
        ...
        game.user.setFlag("D35E", "compendium", data.compendium);

        let avatars = [];
        for (let i = 1; i <= 3; i++) {
            if (data["avatar-folder"][i])
                avatars.push(data["avatar-folder"][i]);
            game.user.setFlag("D35E", "avatar" + i, data["avatar-folder"][i]);
        }
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            game.user.setFlag("D35E", "token" + i, data["token-folder"][i]);
        }

        if (data["sound-folder"])
            game.user.setFlag("D35E", "sound-folder", data["sound-folder"]);
                ...
                for (var i = 0; i < entries.length; i++) {
                    ...
                    for (let name of names) {
                        for (let type of mtype) {
                            ...
                            let result = await this.fixEntry(entry, "flags.D35E.sound-effect", root, soundname, "sound", options.wildcard);
                            if (!!result) {
                                if (result !== true) {
                                    mergeObject(update, result);
                                }
