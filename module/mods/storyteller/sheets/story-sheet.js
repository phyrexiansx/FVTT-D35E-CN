...
export class StorySheet extends JournalSheet {
    pageFlip = "systems/D35E/module/mods/storyteller/sounds/paper-flip.mp3"
    ...
    static get defaultOptions() {
        ...
        return foundry.utils.mergeObject(super.defaultOptions, {
            ...
            template: 'systems/D35E/module/mods/storyteller/templates/story-sheet.html',
            width: getBookWidth(),
    ...
    async _render(force, options = {}) {
        ...
        // [D35E]翻页容错：savedPage 可能为 0/null/"0"（设置残留），统一归一化并限制在有效页范围内
        let savedPage = Number(getPage(data._id)) || 1
        if (savedPage < 1 || savedPage > data.pages.length) {
            savedPage = startPage
        }

        // [D35E]空书保护：没有页面元素时跳过 turn 初始化（否则报 "page 0 does not exist"）
        const bookEl = document.getElementById('story-' + data._id)
        if (!bookEl || !bookEl.querySelector('.page-num')) {
            return
        }
