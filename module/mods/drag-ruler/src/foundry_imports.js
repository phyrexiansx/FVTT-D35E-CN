...
async function animateEntities(entities, draggedEntity, draggedRays, wasPaused) {
	...
	if (!newRays.length) return; // D35E补丁：无有效路径直接结束
	const entityAnimationData = entities.map(entity => {
		const entityOffset = calculateEntityOffset(entity, draggedEntity) || {x: 0, y: 0}; // D35E补丁
		const offsetRays = newRays.map(ray => applyOffsetToRay(ray, entityOffset));

		// Determine offset relative to the Token top-left.
		// This is important so we can position the token relative to the ruler origin for non-1x1 tokens.
		const firstWaypoint = this.waypoints.find(w => !w.isPrevious) || this.waypoints[0]; // D35E补丁
	...
	const startWaypoint = animate ? 0 : Math.max(0, entityAnimationData[0].rays.length - 1); // D35E补丁
	...
	for (let i = startWaypoint; i < entityAnimationData[0].rays.length; i++) {
		...
		const entityPaths = entityAnimationData.map(({entity, rays, dx, dy}) => {
			const ray = rays[i];
						if (!ray?.B) return {entity, path: null}; // D35E补丁：该段路径缺失则跳过
			const dest = [ray.B.x, ray.B.y];
			const path = new Ray({x: entity.x, y: entity.y}, {x: dest[0] + dx, y: dest[1] + dy});
			return {entity, path};
		});
		const updates = entityPaths
			.filter(({path}) => path) // D35E补丁：过滤空路径
			.map(({entity, path}) => {
				return {x: path.B.x, y: path.B.y, _id: entity.id};
			});
		if (!updates.length) continue; // D35E补丁
